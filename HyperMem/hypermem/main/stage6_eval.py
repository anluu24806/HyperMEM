...
async def process_group_responses(
    ...
    for idx, response in enumerate(group_responses):
        ...
        grading_tasks = [
            locomo_grader(oai_client, model_name, question, ground_truth, answer) for _ in range(num_runs)
        ]
...
async def process_single_group(
    ...
    try:
        ...
        result = await process_group_responses(
            group_id, group_responses, oai_client, model_name, options, num_runs, progress, task_id
        )
...
async def main():
    ...
    # --- Evaluation ---
    all_grades = {}
    if config.resume and judged_path.exists():
        try:
            with open(judged_path, encoding="utf-8") as file:
                all_grades = json.load(file)
            console.print(f"[cyan]Loaded existing judged results for resume: {judged_path}[/cyan]")
        except Exception as e:
            console.print(f"[yellow][!] Failed to load existing judged results, starting fresh: {e}[/yellow]")
            all_grades = {}
    ...
    group_ids = sorted(locomo_responses.keys(), key=_group_sort_key)
    if config.limit_conversations > 0:
        group_ids = group_ids[:config.limit_conversations]

    total_responses_count = sum(
        len(locomo_responses.get(group_id, [])) for group_id in group_ids
    )
    console.print(f"[bold]Total responses:[/bold] {total_responses_count} (from {len(group_ids)} conversations)\n")
    ...
    async def process_with_semaphore(group_id: str, group_responses: list, task_id: int, progress: Progress):
        ...
        async with semaphore:
            return await process_single_group(group_id, group_responses, oai_client, llm_config["model"], options, num_runs, progress, task_id)
    ...
    with Progress(
        ...
        for group_id in group_ids:
            ...
            existing_questions = {
                item.get("question") for item in all_grades.get(group_id, []) if item.get("question")
            }
            if existing_questions:
                group_responses = [resp for resp in group_responses if resp.get("question") not in existing_questions]

            active_users += 1
            
            # Create progress task
            task_id = progress.add_task(
                f"[cyan]{group_id}[/cyan]",
        ...
        for result in group_results:
            if isinstance(result, tuple):
                ...
                existing_group = all_grades.get(group_id, [])
                ...
                all_grades[group_id] = list(existing_by_question.values())
