...
def process_single_conversation_retrieval(
    ...
    try:
        ...
        results_for_conv = []
        qa_pairs = conversation_data["qa"][:max_questions] if max_questions and max_questions > 0 else conversation_data["qa"]
        for qa_pair in qa_pairs:
            ...
            # Future optional VLE augmentation goes here.
            ...
            search_duration_ms = (time.time() - query_start) * 1000.0
...
async def main():
    ...
    if not dataset_path.exists():
        console.print(
            "[red]Dataset not found. Set HYPERMEM_DATA_PATH or copy locomo10.json to methods/HyperMem/data/locomo10.json[/red]"
        )
        return
    ...
    if need_emb:
        ...
        try:
            embedding_provider.embed(["embedding smoke test"])
        except Exception as e:
            console.print(
                f"[red][X] Embedding service unavailable: {e}[/red]\n"
                f"[red]Set EMBEDDING_BASE_URL and EMBEDDING_MODEL_NAME to a running Qwen3 embedding service.[/red]"
            )
            raise
    ...
    if config.use_reranker:
        ...
        try:
            reranker_provider.rerank(["question"], ["document"])
        except Exception as e:
            console.print(
                f"[red][X] Reranker service unavailable: {e}[/red]\n"
                f"[red]Set RERANKER_BASE_URL and RERANKER_MODEL_NAME to a running Qwen3 reranker service, or set HYPERMEM_USE_RERANKER=false.[/red]"
            )
            raise
    ...
    with open(dataset_path, "r", encoding="utf-8") as f:
        dataset = json.load(f)
    if config.limit_conversations > 0:
        dataset = dataset[:config.limit_conversations]
    
    console.print(f"[bold]Number of conversations:[/bold] {len(dataset)}\n")
    console.print(f"[bold]Question limit:[/bold] {config.limit_questions if config.limit_questions > 0 else 'all'}")

    all_search_results = {}
    if config.resume and results_output_path.exists():
        try:
            with open(results_output_path, "r", encoding="utf-8") as f:
                all_search_results = json.load(f)
            console.print(f"[cyan]Loaded existing retrieval results for resume: {results_output_path}[/cyan]")
        except Exception as e:
            console.print(f"[yellow][!] Failed to load existing search results, starting fresh: {e}[/yellow]")
            all_search_results = {}
    ...
    with Progress(
        ...
        for i, conversation_data in enumerate(dataset):
            ...
            if config.limit_questions > 0:
                query_count = min(query_count, config.limit_questions)
            conv_id_str = f"locomo_exp_user_{i}"
            if config.resume and config.skip_existing and conv_id_str in all_search_results:
                console.print(f"[cyan][√] Conv {i}: existing search results found, skipping[/cyan]")
                continue
