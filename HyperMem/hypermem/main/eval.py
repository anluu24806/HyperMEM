...
Examples:
  ...
  python eval.py --list             # List all stages
    ...
    parser.add_argument(
        ...
        help="Experiment name override (sets HYPERMEM_EXPERIMENT_NAME)"
    )

    parser.add_argument(
        "--limit_conversations", "--limit-conversations",
        dest="limit_conversations",
        type=int,
        help="Limit number of conversations for debug runs"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help="Limit number of questions per conversation for debug runs"
    )

    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from existing per-stage outputs when supported"
    )
    ...
    parser.add_argument(
        ...
        dest="skip_existing",
        action="store_true",
        help="Skip existing per-conversation artifacts when supported"
    )
    ...
    if args.experiment_name:
        os.environ["HYPERMEM_EXPERIMENT_NAME"] = args.experiment_name
    if args.limit_conversations is not None:
        os.environ["HYPERMEM_LIMIT_CONVERSATIONS"] = str(args.limit_conversations)
    if args.limit_questions is not None:
        os.environ["HYPERMEM_LIMIT_QUESTIONS"] = str(args.limit_questions)
    if args.resume:
        os.environ["HYPERMEM_RESUME"] = "true"
    if args.skip_existing:
        os.environ["HYPERMEM_SKIP_EXISTING"] = "true"
