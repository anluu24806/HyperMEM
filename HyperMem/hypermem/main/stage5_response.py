...
async def process_qa(qa, search_result, oai_client, llm_config, experiment_config):
    ...
    response_data = {
        ...
        "search_duration_ms": search_result.get("search_duration_ms", search_result.get("duration_ms", 0)),
    }
...
async def main(search_path, save_path):
    ...
    llm_config = experiment_config.llm_config[experiment_config.llm_service]
    oai_client = AsyncOpenAI(
        api_key=llm_config["api_key"], base_url=llm_config["base_url"], timeout=llm_config.get("timeout", 120)
    )
    ...
    group_ids = sorted(locomo_search_results.keys(), key=_group_sort_key)
    if experiment_config.limit_conversations > 0:
        group_ids = group_ids[:experiment_config.limit_conversations]

    console.print(f"[bold]Number of conversations:[/bold] {len(group_ids)}")
    console.print(f"[bold]Answer type:[/bold] {experiment_config.answer_type}\n")

    all_responses = {}
    if experiment_config.resume and save_path.exists():
        try:
            with open(save_path, encoding="utf-8") as file:
                all_responses = json.load(file)
            console.print(f"[cyan]Loaded existing responses for resume: {save_path}[/cyan]")
        except Exception as e:
            console.print(f"[yellow][!] Failed to load existing responses, starting fresh: {e}[/yellow]")
            all_responses = {}
    ...
    for group_id in group_ids:
        ...
        if experiment_config.limit_questions > 0:
            qa_set_filtered = qa_set_filtered[:experiment_config.limit_questions]

        search_results = locomo_search_results.get(group_id)
        existing_questions = {
            item.get("question") for item in all_responses.get(group_id, []) if item.get("question")
        }

        matched_pairs = []
        for qa in qa_set_filtered:
            question = qa.get("question")
            if experiment_config.resume and question in existing_questions:
                continue
            ...
            if matching_result:
                ...
                console.print(f"[yellow][!] Conversation {group_id}: No matching search result found - {question[:50]}...[/yellow]")
    ...
    with Progress(
        ...
        for result in results:
            if isinstance(result, tuple) and len(result) == 3:
                ...
                existing_group = all_responses.get(group_id, [])
                existing_by_question = {item.get("question"): item for item in existing_group if item.get("question")}
                for response in responses:
                    existing_by_question[response.get("question")] = response
                all_responses[group_id] = list(existing_by_question.values())
                all_token_usages[group_id] = token_usages
