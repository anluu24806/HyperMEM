...
async def build_bm25_index_for_hypergraph(
    ...
    limit_conversations = config.limit_conversations if config.limit_conversations > 0 else config.num_conv
    ...
    if progress:
        ...
        tasks = []
        for i in range(limit_conversations):
...
async def main():
    ...
    skip_existing = config.skip_existing  # True: skip existing files; False: force regeneration of all indexes
    ...
    console.print(f"[bold]Number of conversations:[/bold] {config.limit_conversations if config.limit_conversations > 0 else config.num_conv}")
